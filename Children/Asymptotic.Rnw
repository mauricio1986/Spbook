...
% \begin{equation}
%  \begin{aligned}
%   \E\left|\eta\right|  & =  \E\left(\left|\vepsi^\top\mA\vepsi\right|\right) \\
%                          & =  \E\left(\left|\sum_{i = 1}^n\sum_{j = 1}^na_{ij}\epsilon_i\epsilon_j\right|\right) \\
%                          & \leq  \E\left(\sum_{i = 1}^n\sum_{j = 1}^n\left|a_{ij}\epsilon_i\epsilon_j\right|\right) \\
%                          & = \E\left(\sum_{i = 1}^n\sum_{j = 1}^n\left|a_{ij}\right|\left|\epsilon_i\right|\left|\epsilon_j\right|\right) \\
%                          & \leq \left(\sum_{i = 1}^n\sum_{j = 1}^n\left|a_{ij}\right|\E\left|\epsilon_i\right|\E\left|\epsilon_j\right|\right) \\
%                          & \leq \left(\sum_{i = 1}^n\sum_{j = 1}^n\left|a_{ij}\right|\sigma_i\sigma_j\right)
%  \end{aligned}  
% \end{equation}
